                    ...
                    if not isCooldown then
                        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
                            if hasItem then
                                ...
                                exports["nk-memorygame"]:thermiteminigame(Config.CorrectBlocks, Config.IncorrectBlocks, Config.TimeToShow, Config.TimeToLose,
                                ...
                                QBCore.Functions.Notify("You Don't Have The Right Equipment!"), "error", 3500)
                                --QBCore.Functions.Notify("You Don\'t Have The Correct Equipment!", "error")   
                            end
                        end, "thermite")
                    else
                        QBCore.Functions.Notify("This Has Just Been Hit, You'll Have To Wait!"), "error", 3500)
                        --QBCore.Functions.Notify("This Has Just Been Hit, You'll Have To Wait!", "error")
                    end      
                end)
            else
                QBCore.Functions.Notify("Not Enough Police On Duty..."), "error", 3500)
        ...
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
            --if Config.JewelLocation["ThermiteSecurity"].isDone then
                if hasItem then
                    ...
                    SecurityAnimation()
                    Wait(2000)
                    exports['nk-hackingame']:OpenHackingGame(Config.VangelicoTime, Config.VangelicoBlocks, Config.VangelicoRepeat, 
                    function(Success)
                        if Success then
                            SecuritySuccess()
                            SecuritySuccessAnim()
            ...
            --else
                --QBCore.Functions.Notify("error.cooldown_disable"), "error", 3500)
...
function ThermiteFailed()
    QBCore.Functions.Notify("You Failed To Unlock The Front Door"), "error", 3500)
...
function ThermiteSuccess()
    QBCore.Functions.Notify("You Successfully Unlocked The Front Door!"), "success", 3500)
...
function SecurityFailed()
    QBCore.Functions.Notify("You Failed To Disable The Cameras!"), "error", 3500)
...
function SecuritySuccess()
    QBCore.Functions.Notify("You Successfully Disable The Cameras!"), "success", 3500)
...
AddEventHandler('nc-vangelico:client:rebootsystem', function()
    QBCore.Functions.Notify("The System Will Reboot In 30 Seconds!"), "success", 3500)
    if Config.DoorLock == "nui" then
        Citizen.Wait(30000)
